        ...
        >
          ...
          {hoveredIdx === i && src.text_preview && (
            <div className="source-tooltip">
              {src.section && <div style={{ color: '#6c63ff', fontWeight: 600, marginBottom: 4 }}>{src.section}</div>}
              <div style={{ opacity: 0.8 }}>{src.text_preview}</div>
              <div style={{ marginTop: 6, opacity: 0.5 }}>Score: {src.relevance_score.toFixed(3)}</div>
            </div>
          )}
        </div>
      ))}
